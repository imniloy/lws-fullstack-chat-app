...
const ChatBody = () => {
  let { id } = useParams();
  const { data: messages, isLoading, isError, isSuccess } = useGetMessagesQuery(id);
  ...
  if (!isLoading && isError) {
    content = <div className="w-full grid h-full justify-center items-center">
      <span className='font-bold text-red-500'>Network Error! Failed to Fatch</span>
    </div>;
  };
